...
const App: React.FC = () => {
  ...
  const logUserActivity = async (uid: string) => {
    ...
    if (existing) {
      if (existing.count < 3) {
        await supabase
          .from('user_activity')
          .update({ count: existing.count + 1, last_active_at: new Date().toISOString() })
          .eq('id', existing.id);
      }
    } else {
      await supabase
        .from('user_activity')
        .insert([{ uid, activity_date: today, count: 1 }]);
    }

    // 2. 关键：重新获取所有活跃数据以同步前端 Analytics 视图
  ...
  const handleUpdatePassword = async (uid: string, newPass: string) => {
    if (supabase) await supabase.from('users').update({ password: newPass }).eq('uid', uid);
        ...
        <div className="flex flex-1 flex-col overflow-hidden">
          ...
          <main className="flex-1 overflow-y-auto p-4 md:p-6 pb-32">
            <Routes>
              ...
              <Route path="/admin" element={<AdminView users={users} demands={demands} resources={resources} onUpsertUser={handleUpsertUser} onDeleteUser={handleDeleteUser} onUpdateDemands={setDemands} onUpdateResources={setResources} />} />
