...
const DashboardView: React.FC<DashboardViewProps> = ({ user, demands, resources }) => {
  ...
  const myDemands = useMemo(() => demands.filter(d => d.creatorId === user.uid), [demands, user]);
  const myResources = useMemo(() => resources.filter(r => r.owner === user.realName), [resources, user]);
  ...
  return (
    <div className="space-y-6 md:space-y-8 animate-in fade-in duration-500 max-w-7xl mx-auto pb-20 px-1 md:px-0">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-6">
        {stats.map((stat, i) => (
          <div key={i} className="bg-white p-5 md:p-6 rounded-[32px] border border-slate-100 shadow-sm transition-all hover:shadow-md hover:border-nb-red/10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        >
          ...
          <div>
            <h3 className="text-xl font-black text-slate-900">我的需求</h3>
            <p className="text-slate-500 font-bold text-sm mt-1">您共发布了 {myDemands.length} 个协作需求</p>
        ...
        >
          ...
          <div>
            <h3 className="text-xl font-black text-slate-900">我的资源</h3>
            <p className="text-slate-500 font-bold text-sm mt-1">您共分享了 {myResources.length} 项知识资产</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
        <div className="lg:col-span-2 space-y-6">
          <div className="flex items-center justify-between px-1">
            <h2 className="text-xl font-black text-slate-800">广场最新动态</h2>
          ...
          <div className="space-y-3 md:space-y-4">
            {demands.filter(d => d.status !== DemandStatus.COMPLETED).slice(0, 5).map((item) => (
              <div 
                key={item.did} 
                onClick={() => navigate(`/demands/${item.did}`)}
                className="bg-white p-4 md:p-5 rounded-[28px] border border-slate-100 shadow-sm flex items-center gap-4 group cursor-pointer active:scale-[0.98] transition-all hover:border-nb-red/20"
              >
                <img src={item.creatorAvatar} className="h-10 w-10 rounded-full md:h-14 md:w-14 ring-4 ring-slate-50 shadow-sm object-cover" alt="" />
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1.5 overflow-hidden">
                    <span className={`text-[8px] md:text-[9px] font-black px-2 py-0.5 rounded-lg shrink-0 ${TAG_COLORS[item.tags[0]] || 'bg-slate-100'}`}>{item.tags[0]}</span>
                    <h3 className="font-black text-slate-900 text-sm md:text-lg truncate group-hover:text-nb-red transition-colors">{item.title}</h3>
                  </div>
                  <div className="flex items-center gap-2 text-[10px] text-slate-400 font-bold uppercase tracking-tight">
                    <span className="text-slate-600">{item.creatorName}</span>
                    <span className="opacity-30">|</span>
                    <span>{item.createdAt}</span>
                  </div>
                </div>
                <div className="text-right shrink-0 bg-slate-50 px-3 md:px-5 py-2 md:py-3 rounded-[20px] shadow-inner border border-slate-100">
                  <p className="text-sm md:text-xl font-black text-nb-red tracking-tighter">{item.rewardValue}</p>
                  <p className="text-[8px] md:text-[10px] text-slate-400 font-black uppercase tracking-widest">{item.rewardType === 'POINTS' ? 'P' : '¥'}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between px-1">
            <h2 className="text-xl font-black text-slate-800">支行智库</h2>
            <button onClick={() => navigate('/resources')} className="text-xs font-black text-slate-400 hover:text-nb-red transition-colors p-2">浏览全部</button>
          </div>
          <div className="bg-slate-900 rounded-[40px] overflow-hidden shadow-2xl p-6 md:p-10 relative group">
            <div className="absolute -top-10 -right-10 w-40 h-40 bg-nb-red/20 blur-[80px] rounded-full group-hover:bg-nb-red/30 transition-all duration-500"></div>
              ...
              {resources.slice(0, 5).map((res) => (
                ...
                >
                  <div className="min-w-0 pr-4">
                    <p className="text-sm font-black text-white group-hover/item:text-nb-red truncate transition-colors leading-tight mb-1">{res.title}</p>
                    <p className="text-[9px] text-slate-500 font-bold uppercase tracking-[0.2em]">{res.type}</p>
                  </div>
                  <div className="h-8 w-8 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center group-hover/item:bg-nb-red group-hover/item:border-nb-red transition-all shrink-0">
