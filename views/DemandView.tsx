...
import { TAG_COLORS, DEMAND_TAGS } from '../constants';
...
const DemandView: React.FC<DemandViewProps> = ({ user, demands }) => {
  ...
  const filteredDemands = useMemo(() => {
    ...
    switch (activeTab) {
      ...
      case 'COMPLETED':
        ...
        break;
      default:
        // 'ALL' shows all except completed normally, or you might want all
  ...
  // Map backend status to user-friendly list labels
    ...
    <div className="max-w-7xl mx-auto space-y-6 animate-in fade-in duration-300 pb-24 px-1 md:px-0">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 px-1">
        <div>
          ...
          <p className="text-slate-500 text-sm font-bold mt-1">跨部门高效联动，解决客户最后一公里难题。</p>
        </div>
        <button 
          onClick={() => navigate('/demands/upload')}
          className="bg-nb-red text-white px-8 py-4 rounded-2xl font-black flex items-center justify-center gap-3 shadow-2xl shadow-red-200 active:scale-95 transition-all w-full md:w-auto text-sm"
        >
          <Plus size={20} />
          发布需求
        </button>
      ...
      <div className="bg-white rounded-[32px] p-4 md:p-6 border border-slate-100 shadow-sm space-y-4 mx-1">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">标签筛选</span>
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {Object.keys(DEMAND_TAGS).map(tag => (
            <button
              ...
              className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase transition-all border ${
                selectedL1 === tag ? 'bg-nb-red text-white border-nb-red' : 'bg-white text-slate-500 border-slate-200'
              }`}
            >
              {tag}
            </button>
      ...
      <div className="grid grid-cols-3 md:flex md:flex-wrap gap-2 p-2 bg-white border border-slate-100 rounded-[24px] shadow-sm sticky top-16 z-30 mx-1">
        {[
          { id: 'ALL', label: '进行中', icon: <LayoutGrid size={14} /> },
          { id: 'URGENT', label: '象山急事', icon: <Zap size={14} /> },
          { id: 'HIGH_REWARD', label: '大额悬赏', icon: <Coins size={14} /> },
          ...
          <button
            ...
            className={`flex flex-col md:flex-row items-center justify-center gap-1.5 px-3 py-2.5 rounded-xl text-[10px] md:text-xs font-black transition-all uppercase tracking-tight ${
              activeTab === tab.id ? 'bg-slate-900 text-white shadow-lg' : 'text-slate-400 hover:text-slate-600 active:bg-slate-50'
            }`}
          >
            {tab.icon}
        ...
        {filteredDemands.length > 0 ? filteredDemands.map(demand => {
          ...
          return (
            <div 
              ...
              className={`bg-white rounded-[40px] p-6 md:p-8 border border-slate-100 shadow-sm hover:border-nb-red/30 hover:shadow-2xl transition-all group cursor-pointer active:scale-[0.99] relative overflow-hidden ${demand.status === DemandStatus.COMPLETED ? 'bg-slate-50/50' : ''}`}
            >
              <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 mb-8">
                <div className="flex-1 min-w-0">
                  <div className="flex flex-wrap items-center gap-2 mb-4">
                    {demand.isRecommended && (
                      <span className="bg-yellow-400 text-slate-900 text-[9px] font-black px-3 py-1 rounded-full uppercase flex items-center gap-1 shadow-sm">
                        <Star size={10} fill="currentColor"/> 行长推荐
                      </span>
                    )}
                    <span className={`text-[9px] font-black px-3 py-1 rounded-full uppercase tracking-widest ${demand.urgency === DemandUrgency.EXTREME ? 'bg-nb-red text-white animate-pulse' : demand.urgency === DemandUrgency.URGENT ? 'bg-red-100 text-red-600' : 'bg-slate-100 text-slate-500'}`}>
                      {demand.urgency}
                    </span>
                    <span className={`text-[9px] font-black px-3 py-1 rounded-full uppercase tracking-widest ${TAG_COLORS[demand.tags[0]] || 'bg-slate-50 text-slate-400'}`}>{demand.tags[0]}</span>
                  </div>
                  <h3 className="text-lg md:text-2xl font-black text-slate-900 group-hover:text-nb-red transition-colors line-clamp-1 leading-tight tracking-tight">{demand.title}</h3>
                  <p className="text-xs md:text-base text-slate-500 mt-3 line-clamp-2 leading-relaxed font-medium">{demand.customerInfo} · {demand.description}</p>
                </div>
                <div className={`shrink-0 px-6 py-5 rounded-[32px] flex flex-col items-center justify-center min-w-[100px] border shadow-inner ${demand.rewardType === 'POINTS' ? 'bg-yellow-50 border-yellow-100 text-yellow-700' : 'bg-green-50 border-green-100 text-green-700'}`}>
                  <span className="text-[10px] font-black uppercase tracking-[0.2em] opacity-60 mb-1">{demand.rewardType === 'POINTS' ? '积分' : '奖励'}</span>
                  <span className="text-2xl md:text-3xl font-black tracking-tighter">{demand.rewardValue}</span>
                </div>
              </div>
              
              <div className="flex items-center justify-between pt-6 border-t border-slate-50">
                <div className="flex items-center gap-3">
                  <img src={demand.creatorAvatar} className="h-10 w-10 rounded-full ring-4 ring-slate-50 shadow-sm object-cover" alt="" />
                  <div>
                    <p className="text-xs font-black text-slate-900 leading-none mb-1">{demand.creatorName}</p>
                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">{demand.createdAt}</p>
                  </div>
                </div>
                <div className={`flex items-center gap-2 font-black text-[11px] px-4 py-2 rounded-2xl border ${statusInfo.color} uppercase tracking-[0.1em] shadow-sm`}>
                  {statusInfo.icon}
                  {statusInfo.label}
                </div>
              </div>
          ...
          <div className="bg-white rounded-[40px] p-24 text-center border-2 border-dashed border-slate-200 flex flex-col items-center gap-4">
            <div className="h-20 w-20 bg-slate-50 rounded-full flex items-center justify-center text-slate-300">
               <LayoutGrid size={40} />
            </div>
            <p className="text-slate-400 font-black uppercase tracking-widest text-sm">暂无符合条件的协作需求</p>
          </div>
        )}
      </div>
    </div>
